...
resource "aws_route_table" "data" {
  ...
  tags = {
    Name = "${var.project_name}-data-rt-${count.index + 1}-${var.environment}"
  }
}
...
resource "aws_route_table" "kafka" {
  ...
  tags = {
    Name = "${var.project_name}-kafka-rt-${count.index + 1}-${var.environment}"
  }
}
...
resource "aws_route_table_association" "kafka" {
  ...
  route_table_id = aws_route_table.kafka[count.index].id
}
